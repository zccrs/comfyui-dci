...
**节点类别**：`DCI/Files`
**功能描述**：从文件系统加载二进制文件，专为处理 DCI 图标文件等二进制数据设计。支持通过其他节点输入文件路径，提供更灵活的工作流程。

**可选输入参数：**
- **`file_path`** (STRING)：要加载的文件路径，可通过其他节点输入或手动输入，默认空字符串
...
**节点类别**：`DCI/Preview`
**功能描述**：直接在节点内显示 DCI 文件内容的可视化网格预览和元数据信息。无需外部输出，所有预览内容都在节点界面中展示。支持文件路径和二进制数据两种输入方式。
