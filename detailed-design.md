...
- `show_file_paths`: 是否显示文件路径列表 (布尔值，可选，默认True)

**节点内显示内容**:
- **图像网格预览**: 自动生成的网格布局，图像居左对齐显示
- **可调节文本显示**: 支持8-24像素的字号设置，使用HTML格式化，便于在不同分辨率下查看
...
- 可调节文本字号，支持8-24像素范围，使用HTML格式化确保兼容性
- 文件路径显示控制，用户可选择是否显示完整的DCI内部路径列表
- 移除了 `dci_file_path` 参数，专注于二进制数据输入
- 简化了节点接口，提高了易用性
